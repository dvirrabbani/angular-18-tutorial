...
export class HousingService {
  ...
  async getAllHousingLocations(): Promise<HousingLocation[]> {
    try {
      ...
      return this.fallbackHousingLocations
    }
  }
  async getHousingLocationById(id: number): Promise<HousingLocation | undefined> {
    try {
      const data = await fetch(`${this.url}/${id}`);
      return (await data.json()) ?? this.fallbackHousingLocations.find(fallbackHousingLocation => fallbackHousingLocation.id === id);

    } catch (error) {
      return this.fallbackHousingLocations.find(fallbackHousingLocation => fallbackHousingLocation.id === id)
    }
  }
